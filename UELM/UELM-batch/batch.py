...
def run():
    ...
    for i in range(len(output)):
        ...
        for j in range(len(output[i])):
            ...
            max_token = max(max_token,output[i][j].value)
        #print("query:",query)
        response,outputs,inputs = model.chat_batch(tokenizer, query, historys,min(max_token+500,1024))
        end = time.time()
        t = end-start
