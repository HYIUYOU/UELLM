...
def groupPrograms(querys):
    ...
    sorted_querys = sorted(querys, key=lambda x: x.value)
    ...
    for q in sorted_querys:
        ...
        if len(current_group)<batch_size and (q.value + current_max) <= THRESHOLD: # 0.1 is the estimated additional consumption in parallel
            ...
            current_max = q.value + current_max
...
def run():
    ...
    model = load_model_on_gpus("/path/to/model", num_gpus=4)
    ...
    for i in range(len(output)):
        ...
        for j in range(len(output[i])):
            ...
            if output[i][j].SLO < t:
                l += 1
        # print("There are ",i+1," groups in total.")
