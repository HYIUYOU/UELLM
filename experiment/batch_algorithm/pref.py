...
def monitor_and_record_gpus(duration=10, interval=1, filename='/path/to/data'):
    ...
    with open(filename, 'w', newline='') as csvfile:
        ...
        csvwriter.writerow(fields)  # Write header of the table
...
def plot_data(data_records, fields):
    ...
    for i in range(4):
        ...
        plt.legend()
        plt.grid(True)
        plt.show()  # Displays a graph of the current GPU and waits for closure to continue

    # Plotting Memory Change
    plt.figure(figsize=(10, 4))
...
def plot_data2(data_records, fields):
    ...
    # Create a window with four subgraphs showing the utilization of the four GPUs
    ...
    plt.show()  # Displays a window showing the utilization of the four GPUs
...
if __name__ == "__main__":
    monitor_and_record_gpus(duration=1000)  # Monitor for 60 seconds
