...
def parse_args():
    ...
    parser.add_argument("--tensorboard",default="/path/to/tensorboard",type=str)
    # deepspeed features
    ...
    parser.add_argument('--dataset_length',type=int,default=1500,help='')
...
def main():
    ...
    train_dataset = MODE[args.mode]["rawdataset"](args.train_path, tokenizer, args.max_len, args.max_src_len, args.is_skip)
    train_dataset = train_dataset[:args.dataset_length] 
    if args.local_rank == -1:
        ...
        for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader), unit="batch"):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.save_model_step is not None and global_step % args.save_model_step == 0:
                    # If training with zero3, model parameters need to be merged and saved
